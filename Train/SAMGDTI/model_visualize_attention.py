...
class CAN_Layer(nn.Module):
    ...
    def forward(self, protein, drug, mask_prot, mask_drug, return_attn=False):
        ...
        if return_attn:
            return query_embed, alpha_pd, alpha_dp, alpha_pp, alpha_dd
        else:
            return query_embed
...
class ColdstartCPI(nn.Module):
    ...
    def forward(self, input_tensors, return_attn=False):
        ...
        if return_attn:
            joint_embed, alpha_pd, alpha_dp, alpha_pp, alpha_dd = self.can_layer(p_m_pocket_sa, c_m, p_mask_pocket_sa, c_mask, return_attn=True)  # 口袋token joint_embed(B,unify_num*2) p_m_sa(B,512,512), c_m(B,300,512)
        else:
        ...
        if return_attn:
            return predict, alpha_pd, alpha_dp, alpha_pp, alpha_dd
        else:
            return predict
